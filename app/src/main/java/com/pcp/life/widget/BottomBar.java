...
public class BottomBar extends View {
    ...
    //监听点击事件
    private IOnTabSelected onTabSelected = null;

    public BottomBar selectedListener(IOnTabSelected selected) {
...
//        return bitmapDrawable.getBitmap();
    ...
    protected void switchFragment(int whichFragment) {
        ...
        if (fragment != null) {
            ...
            if (fragment.isAdded()) {
                ...
                //监听Fragment初始化后的重新点击事件
                if (onTabSelected != null) {
                    onTabSelected.onTabSelectedAfterInit(whichFragment, fragment);
                }
            } else {
                if (currentFragment != null) {
    ...
    //该类是在Fragment已经初始化后才调用的
    // 所以不用判断Fragment是否为null 或者 存不存在
    public interface IOnTabSelected{

        void onTabSelectedAfterInit(int position, Fragment fragment);
    }
}

...
import com.pcp.life.mvvm.bean.book.ListBookInfo;
...
public class Book_Refresh_LoadMoreUtil {
    ...
    public static void refresh_or_firstRequest(Context context, String baseUrl, int custom_count,
        ...
        refreshLayout.setOnRefreshListener(new OnRefreshListener() {
            ...
            public void onRefresh(@NonNull RefreshLayout refresh) {
                ...
                OkGo.<ListBookInfo>get(baseUrl + "&start=" + start + "&count=" + count).tag(context).execute(new JsonCallback<ListBookInfo>() {
                    @Override
                    public void onSuccess(Response<ListBookInfo> response) {

                        if (response.body().getBooks().size() > 0 && refreshListener != null) {
                            //将数据往外抛，让外部处理
                            refreshListener.onSuccessToRefresh(response.body().getBooks());
                    ...
                    @Override
                    public void onError(Response<ListBookInfo> response) {
                        refresh.finishRefresh(false);//结束刷新（刷新失败）
        ...
        refreshLayout.setOnLoadMoreListener(new OnLoadMoreListener() {
            ...
            public void onLoadMore(RefreshLayout loadMore) {
                ...
                OkGo.<ListBookInfo>get(baseUrl + "&start=" + start + "&count=" + count).tag(context).execute(new JsonCallback<ListBookInfo>() {
                    @Override
                    public void onSuccess(Response<ListBookInfo> response) {
                        if (response.body().getBooks().size() > 0 && loadMoreListener != null) {
                            //将数据往外抛，让外部处理
                            loadMoreListener.onSuccessToLoadMore(response.body().getBooks());

...
import com.pcp.life.mvvm.vm.item_vm.movie.RecycleRecentMovieItemVM;
...
public class SCMovie_RecentViewModel extends BaseViewModel {



    public SCMovie_RecentViewModel(@NonNull Application application) {
        super(application);

    }

    //BaseUrl
    public static String baseUrl = "https://api.douban.com/v2/movie/in_theaters?";
    //每页的数据
    ...
    public ItemBinding<ItemViewModel> itemBinding = ItemBinding.of(BR.recentMovieItem, R.layout.recycle_movie_recent);
    ...
    public void requestRecentMovie(SmartRefreshLayout smartRefreshLayout) {
    ...
    private void setMovieInfo(List<Movie> movies) {

        for (Movie entity : movies) {
            RecycleRecentMovieItemVM itemViewModel = new RecycleRecentMovieItemVM(SCMovie_RecentViewModel.this, entity);
            //双向绑定动态添加Item
            observableList.add(itemViewModel);
        }
    }

    public void recentMovieLoadMore(SmartRefreshLayout smartRefreshLayout, RecyclerView recyclerView) {

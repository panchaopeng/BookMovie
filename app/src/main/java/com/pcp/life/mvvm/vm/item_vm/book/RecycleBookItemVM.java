...
public class RecycleBookItemVM extends ItemViewModel<BaseViewModel> {

    public ObservableField<BookInfo> bookInfo = new ObservableField<>();

    public RecycleBookItemVM(@NonNull BaseViewModel viewModel, BookInfo entity) {
        super(viewModel);
    ...
    //android:rating="@{popularItem.bookInfo.rating.average/2}"
    public float getRating(){

        return bookInfo.get().getRating().getAverage() / 2 ;
    }

    //作者 / 出版日期 / 出版社
    public String getConcactText(){
        String str = BookDetailUtil.formatListToString(bookInfo.get().getAuthor()) +" / "+
                bookInfo.get().getPubdate()+" / "+bookInfo.get().getPublisher();
        return str;
    }

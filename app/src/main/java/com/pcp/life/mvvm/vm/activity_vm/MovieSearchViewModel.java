...
public class MovieSearchViewModel extends BaseViewModel {
    ...
    public static String movieSearchUrl = "https://api.douban.com/v2/movie/search?q=";
    //每页的数据
    ...
    public void getMoiveList(SmartRefreshLayout smartRefreshLayout,String query){

        //第一次请求数据或者下拉刷新
        smartRefreshLayout.autoRefresh();
        Movie_Refresh_LoadMoreUtil.refresh_or_firstRequest(AppManager.getAppManager().currentActivity(),
                movieSearchUrl + query, count.get(), start.get(),
                smartRefreshLayout,
    ...
    public void loadMore(SmartRefreshLayout smartRefreshLayout, RecyclerView recyclerView,String query) {

        Movie_Refresh_LoadMoreUtil.loadMore(AppManager.getAppManager().currentActivity(),
                movieSearchUrl + query, count.get(),
                smartRefreshLayout,
    ...
    private void setMovieInfo(List<Movie> movies) {
        ...
        for (Movie entity : movies) {
            RecycleRecentMovieItemVM itemViewModel = new RecycleRecentMovieItemVM(MovieSearchViewModel.this, entity);
            //双向绑定动态添加Item
